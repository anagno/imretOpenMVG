...
void USAC<ProblemType>::initPROSAC()
{
	...
	for (size_t n = usac_min_sample_size_+1; n <= usac_num_data_points_; ++n)
	{
		...
		pn_i_vec[usac_min_sample_size_] = (prosac_beta_)*pow((double)1-prosac_beta_, (double)n-usac_min_sample_size_-1)*(n-usac_min_sample_size_);
		...
		for (size_t i = usac_min_sample_size_+2; i <= n; ++i)
		{
			...
			if (i == n)
			{
				pn_i_vec[n-1] = pow((double)prosac_beta_, (double)n-usac_min_sample_size_);
				break;
			}
